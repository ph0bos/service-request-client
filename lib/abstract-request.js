...
const RETRY_DEFAULTS                  = { retries: 3, minTimeout: 75, maxTimeout: 750 };
...
class AbstractRequest {
   ...
   */
  constructor(options = {}) {
    if (new.target === AbstractRequest) {
      throw new TypeError("Cannot construct AbstractRequest instances directly, please extend");
    }
    ...
    this.request.timeout  = options.timeout || REQUEST_TIMEOUT_MS;

    // Retry
    this.retry            = {};
    this.retry.retries    = (options.retries || options.retries >= 0) ? options.retries : RETRY_DEFAULTS.retries;
    this.retry.minTimeout = options.minTimeout || RETRY_DEFAULTS.minTimeout;
    this.retry.maxTimeout = options.maxTimeout || RETRY_DEFAULTS.maxTimeout;
  }
   ...
   * @returns {Promise}
   */
  method(method, uri, options, body) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this._request(method, uri, options, body, (err, res) => {
   ...
   * @private
   */
  async _request(method, uri, options = {}, body, callback) {
    let headers = {};
    let query   = {};
    ...
    operation.attempt(async (attemptCount) => {
      // Inherit
      const baseUrl = await this.resolveServiceBaseURL();

      const url = `${baseUrl}/${uri}`;

      const request = this._applyRequestDefaults(unirest(method, url, headers, body));

      //if (operation.attempts() >= self.retry.retries) return callback(new ZookeeperError('client not connected'));
